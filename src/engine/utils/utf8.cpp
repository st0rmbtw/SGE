...
uint32_t sge::next_utf8_codepoint(const char* text, std::size_t& index) {
    ...
    int cplen = 1;
    if ((text[index] & 0xf8) == 0xf0) {
        cplen = 4;

        const uint8_t b1 = (uint8_t) text[index + cplen - 1];
        const uint8_t b2 = (uint8_t) text[index + cplen - 2];
        const uint8_t b3 = (uint8_t) text[index + cplen - 3];
        const uint8_t b4 = (uint8_t) text[index + cplen - 4];
