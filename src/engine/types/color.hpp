...
struct Hsla {
    ...
    static constexpr Hsla white() {
        return Hsla(0.0f, 0.0f, 1.0f, 1.0);
    }
    ...
    inline operator LinearRgba() const {
        return to_linear_rgba();
    }
...
inline LinearRgba detail::hsla_to_linear_rgba(const Hsla& hlsa) {
    ...
    if (hlsa.saturation == 0.0f) {
        ...
        const float q = hlsa.lightness < 0.5 ? hlsa.lightness * (1.0f + hlsa.saturation) : hlsa.lightness + hlsa.saturation - hlsa.lightness * hlsa.saturation;
        const float p = 2.0f * hlsa.lightness - q;
        result.red = hue2rgb(p, q, hlsa.hue + 1.0f / 3.0f) * 255.0f;
        result.green = hue2rgb(p, q, hlsa.hue) * 255.0f;
        result.blue = hue2rgb(p, q, hlsa.hue - 1.0f / 3.0f) * 255.0f;
    }
    return result;
}
