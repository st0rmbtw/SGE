...
std::vector<LLGL::VertexAttribute> sge::Attributes::ToLLGL() const {
    ...
    uint32_t location = m_start_location;
    ...
    for (const Attribute& item : m_items) {
        ...
        items.emplace_back(item.name, item.format, location, offset, 0, item.slot, instance_divisor);
